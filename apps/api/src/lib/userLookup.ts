 ...
 *
 * `requireAuthenticatedUser`(기존 인증 미들웨어)와 `POST /api/sessions`(구 `/api/auth/verify-email`)이
...
export async function lookupUserByEmail(email: string): Promise<LookupUserResult> {
  ...
  if (error) {
    return { user: null, isFailedQuery: true };
  }

  if (!data) {
    return { user: null, isFailedQuery: false };
  }
