 ...
 *
 * `Cookie: dic_visitor_email=...` 값으로 `users` 테이블을 조회해 일치하는 row가 있으면
 * `req.user = { id, email }`를 설정하고 다음 핸들러로 넘긴다. `x-user-email` 헤더는
 * v2 재설계로 하위호환 없이 완전히 제거되었다(스펙 결정 #4).
 *
 * 이 미들웨어는 "누가 요청을 허용받는가"만 판단한다. 가드 페이지 렌더링, 쿠키 발급 등
 * 프론트엔드 부분은 별도 기능(이메일 기반 방문자 게이트)의 책임이다.
 *
 * `lookupUserByEmail`(`POST /api/sessions`과 공유하는 헬퍼)로 내부 조회를 위임한다.
 * `isFailedQuery: true`(DB 조회 자체 실패)인 경우에도 여기서는 기존과 동일하게
